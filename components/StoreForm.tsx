  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <input name="imageUrl" type="url" className={INPUT_NORMAL} placeholder="https://..." />
      </div>

      {/* サブ写真 最大5枚 */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">写真URL（最大5枚）</label>
        <div className="space-y-2">
          {[1, 2, 3, 4, 5].map((i) => (
            <input
              key={i} name={`photo${i}`} type="url"
              className={INPUT_NORMAL}
              placeholder={`写真${i} URL (https://...)`}
            />
          ))}
        </div>
      </div>
