...
- ロール選択：一般ユーザー / オーナー
- **お名前**（表示名）の登録 — 必須入力・`user_metadata.display_name` に保存
...
- プロフィールヘッダーに表示名（お名前）＋メールアドレスを表示
- プロフィールヘッダーのペンシルアイコンから設定ページへ直接遷移
- **お名前変更**（`/mypage/change-name`）— 表示名をいつでも更新可能

### オーナー機能（`/mypage/owner`）

- **店舗登録** — 新規店舗の登録（アカウント登録必須）
  - **メイン画像は必須**（新規登録時のみ。未アップロードで送信するとバリデーションエラー）
  ...
  - **登録前プレビュー** — 保存前にモーダルで掲載イメージを確認可能
    - 写真（最大5枚）・SNSリンク・SNS最新投稿・Google Mapsアクセス地図を表示
    - SNS / 投稿URL は入力値をリアルタイムでプレビューに反映（保存前でも確認可能）
...
- **ダッシュボード**
  ...
  - 全店舗一覧テーブル
  - **未対応件数バッジ** — 「店舗審査管理」「お問い合わせ一覧」ボタンに未処理件数を表示
- **店舗審査管理**（`/admin/owners`）
  - 新規登録店舗を審査（3営業日以内）
  - 承認：ワンクリックでオーナーに承認メールを自動送信（ローディングスピナー付き）
  - 否認：テンプレート選択（風俗店舗 / 実店舗ではない / 情報不足 / カスタム）＋否認メール送信（ローディングスピナー付き）
  - メール送信履歴の件数表示
  - 審査済み一覧（審査待ちに戻す機能付き・ローディングスピナー付き）
  - **カラムヘッダークリックでソート** — 店舗名 / カテゴリ / オーナー / 登録日 / ステータス / メール数（昇順・降順切り替え）
- **お問い合わせ対応**（`/admin/contacts`）
  - 問い合わせ一覧（各問い合わせにインラインで返信フォームを表示・送信ローディング付き）
  - 返信時に「〇〇様、いつもNEW OPENをご利用いただきありがとうございます。」を自動付加してメール送信
  - 返信履歴を問い合わせごとに表示
  - **未返信フィルター** — 「すべて」「未返信（N件）」タブで絞り込み、未返信カードは赤ボーダー＋バッジ表示
...
- ログイン状態かつ **お名前・メールアドレスが登録済み** の場合、入力欄をグレーの表示のみに切り替え（hidden input で送信）
- 未ログイン時は名前・メールアドレスを手入力
- 送信ボタンにローディングスピナー（二重送信防止）
...
BASIC_AUTH_PASS=
RESEND_API_KEY=                   # Resend APIキー (re_xxxxxxxxxxxx)
MAIL_FROM=info@newopen.site       # 送信元アドレス（Resendでドメイン認証済みのもの）
MAIL_FROM_NAME=NEW OPEN           # 送信者名（省略時: NEW OPEN）
ADMIN_EMAIL=info@newopen.site     # 管理者通知メール（省略時は MAIL_FROM）
